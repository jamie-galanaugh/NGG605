function[datafluo]=extractionfluo(filename,rig);

%% rig = 0 if you record Rig A and rig=1 
...
filepath=Where;
    ...
    %filepath='C:\Users\edgar\Desktop\dopaminesensor\Group2_Recordings\Synapse\MC_2_M_8-210804-183202\MC2BM8A_0804';
...
datafluo{1,1}=dat1;
datafluo{1,2}=dat2;
datafluo{1,3}=normalit;


datafluo{2,1}=ttlout;
datafluo{2,2}=TTL1diff;
...
% diffTTL1=diff(TTL1);
% diffTTL2=diff(TTL2);
% sampleci=(datas3{3, 5}*40); samplec2i=round(sampleci);samplec2masi=samplec2i+200;samplec2menosi=samplec2i-200;
% dfofCorr_sub2=dfofCorr_sub';
% 
% for k=1:length(datas3{3, 5})
%     if datas3{3, 5}(k,1)>200
%  trialsimpuli(k,:)= dfofCorr_sub2 (samplec2menosi(k,1):samplec2masi(k,1),1);
%  zsctrialsimpuli(k,:)=zscore(trialsimpuli(k,:)); 
%     end
% end
% figure(5)
% image ( zsctrialsimpuli,'CDataMapping','scaled');
% colormap(jet)
% 
% 
% %%  graph imp
% samplec=(datas3{3, 6}*40); samplec2c=round(samplec);samplec2masc=samplec2c+200;samplec2menosc=samplec2c-200;
% dfofCorr_sub2=dfofCorr_sub';
% 
% for k=1:length(datas3{3, 6})
%   if datas3{3, 6}(k,1)>200
%  trialscom(k,:)= dfofCorr_sub2 (samplec2menosc(k,1):samplec2masc(k,1),1);
%  zsctrialscom(k,:)=zscore(trialscom(k,:)); 
%   end
% end
% figure(6)
% image ( zsctrialscom,'CDataMapping','scaled');
% colormap(jet)
% 
% 
% %%   grap comp
% sampleover=(datas3{3, 7} *40); samplec2over=round(sampleover);samplec2masover=sampleover+200;samplec2mensoover=sampleover-200;
% dfofCorr_sub2i=dfofCorr_sub';
% 
% for k=1:length(datas3{3, 7})
%     if datas3{3, 7}(k,1)>200
%  trialsimpuli(k,:)= dfofCorr_sub2 (samplec2mensoover(k,1):samplec2masover(k,1),1);
%  zsctrialsover(k,:)=zscore(trialsimpuli(k,:)); 
%     end
% end
% figure(7)
% image ( zsctrialsover,'CDataMapping','scaled');
% colormap(jet)
% 
% x=1:401;
% figure (8)
% meanzsctrialsover=mean(zsctrialsover);sizesdzsctrialsover=size(zsctrialsover);
% plot(meanzsctrialsover);
% meanzsctrialimp=mean(zsctrialsimpuli);
% figure (9)
% plot(meanzsctrialimp);
% meanzsctrialcorr=mean(zsctrialscom);
% figure (10)
% plot(meanzsctrialcorr);
% sdzsctrialsover=std(zsctrialsover);semzsctrialsover=sdzsctrialsover/sqrt(sizesdzsctrialsover(1,1));
% meanzsctrialsoverm=semzsctrialsover+meanzsctrialsover;
% meanzsctrialsovermen=semzsctrialsover-meanzsctrialsover;
% fill_error_area(x',meanzsctrialsover',meanzsctrialsovermen', meanzsctrialsoverm');
% plot(meanzsctrialsover,'r');
% 
% 
% 
% figure (9)
